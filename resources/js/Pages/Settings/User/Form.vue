    ...
    <el-card body-class="p-0">
        <el-form :model="form" @submit.prevent="onSubmit" label-position="top">
            <div class="p-4">
                <el-row :gutter="20">
                    <el-col :span="12">
                        <upload-image v-model="form.avatar" width="140px" height="140px" 
                        class="mb-5"
                        autoCrop
                        />
                    ...
                    <el-col :span="12">
                        <el-form-item :label="$t('role')" :error="errors.role">
                            <select-role v-model="form.role"/>
                        </el-form-item>
                        <el-form-item :label="$t('branch')" :error="errors.branch_id" v-if="form.role == 2">
                            <select-branch v-model="form.branch_id"/>
                        </el-form-item>
                        <el-form-item :label="$t('password')" :error="errors.password">
                            <el-input v-model="form.password" />
                        </el-form-item>
                        <el-form-item :label="$t('password_confirm')" :error="errors.password_confirmation">
...
const pageTitle = ref(`${ t('create') } ${t('user')}`);
...
const onSubmit = async () => {
    ...
    const url = route.params.id ?
        `/settings/user/${route.params.id}/update` :
        '/settings/user/store';
  try {
